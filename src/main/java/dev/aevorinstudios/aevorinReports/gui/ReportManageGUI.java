...
public class ReportManageGUI {
    ...
    public void open(Player player, Report report) {
        String guiType = plugin
            .getConfig()
            .getString("reports.gui.type", "book")
            .toLowerCase();
        ...
        String title = parsePlaceholders(
            player,
            lang.getMessage(
                "gui.container.manage_report.title",
                Map.of("id", String.valueOf(report.getId()))
            )
        );
        Inventory gui = Bukkit.createInventory(
            new dev.aevorinstudios.aevorinReports.gui.holders.ReportManageHolder(
                report
            ),
            54,
            title
        ...
        if (reportedName == null) reportedName = lang.getMessage(
            "common.unknown"
        );
        ...
        infoMeta.setDisplayName(
            parsePlaceholders(
                player,
                lang.getMessage("gui.container.manage_report.details.title")
            )
        );
        ...
        infoMeta.setLore(
            parsePlaceholders(
                ...
                java.util.List.of(
                    ...
                    lang.getMessage(
                        "gui.container.manage_report.details.lore.reporter",
                        Map.of("reporter", reporterName)
                    ),
                    lang.getMessage(
                        "gui.container.manage_report.details.lore.reported",
                        Map.of("reported", reportedName)
                    ),
                    lang.getMessage(
                        "gui.container.manage_report.details.lore.reason",
                        Map.of(
                            "reason",
                            lang.getLocalizedReason(report.getReason())
                        )
                    ),
                    lang.getMessage(
                        "gui.container.manage_report.details.lore.status",
                        Map.of(
                            "status",
                            lang.getLocalizedStatus(report.getStatus())
                        )
                    ),
                    lang.getMessage(
                        "gui.container.manage_report.details.lore.id",
                        Map.of("id", String.valueOf(report.getId()))
                    ),
                    lang.getMessage(
        ...
        if (player.hasPermission("aevorinreports.manage")) {
            ...
            if (report.getStatus() != Report.ReportStatus.PENDING) {
                ...
                pendingMeta.setDisplayName(
                    parsePlaceholders(
                        player,
                        lang.getMessage(
                            "gui.container.manage_report.pending.title"
                        )
                    )
                );
                pendingMeta.setLore(
                    parsePlaceholders(
                        ...
                        java.util.List.of(
                            lang.getMessage("gui.container.shared.separator"),
                            lang.getMessage(
                                "gui.container.manage_report.pending.lore.description"
                            ),
                            lang.getMessage(
                                "gui.container.manage_report.pending.lore.current_status",
                                Map.of(
                                    "status",
                                    lang.getLocalizedStatus(report.getStatus())
                                )
                            ...
                            lang.getMessage("gui.container.shared.separator"),
                            lang.getMessage(
                                "gui.container.manage_report.pending.lore.action"
                            )
                        )
                    )
                );
            ...
            if (report.getStatus() != Report.ReportStatus.RESOLVED) {
                ...
                resolvedMeta.setDisplayName(
                    parsePlaceholders(
                        player,
                        lang.getMessage(
                            "gui.container.manage_report.resolved.title"
                        )
                    )
                );
                resolvedMeta.setLore(
                    parsePlaceholders(
                        player,
                        java.util.List.of(
                            lang.getMessage("gui.container.shared.separator"),
                            lang.getMessage(
                                "gui.container.manage_report.resolved.lore.description"
                            ),
                            lang.getMessage(
                                "gui.container.manage_report.resolved.lore.current_status",
                                Map.of(
                                    "status",
                                    lang.getLocalizedStatus(report.getStatus())
                                )
                            ...
                            lang.getMessage("gui.container.shared.separator"),
                            lang.getMessage(
                                "gui.container.manage_report.resolved.lore.action"
                            )
                        )
                    )
                );
            ...
            if (report.getStatus() != Report.ReportStatus.REJECTED) {
                ...
                rejectedMeta.setDisplayName(
                    parsePlaceholders(
                        player,
                        lang.getMessage(
                            "gui.container.manage_report.rejected.title"
                        )
                    )
                );
                rejectedMeta.setLore(
                    parsePlaceholders(
                        player,
                        java.util.List.of(
                            lang.getMessage("gui.container.shared.separator"),
                            lang.getMessage(
                                "gui.container.manage_report.rejected.lore.description"
                            ),
                            lang.getMessage(
                                "gui.container.manage_report.rejected.lore.current_status",
                                Map.of(
                                    "status",
                                    lang.getLocalizedStatus(report.getStatus())
                                )
                            ...
                            lang.getMessage("gui.container.shared.separator"),
                            lang.getMessage(
                                "gui.container.manage_report.rejected.lore.action"
                            )
                        )
                    )
                );
        ...
        if (backMeta != null) {
            backMeta.setDisplayName(
                parsePlaceholders(
                    player,
                    lang.getMessage(
                        "gui.container.manage_report.back_button.title",
                        "&c&lBack"
                    )
                )
            );
            backMeta.setLore(
                parsePlaceholders(
                    ...
                    java.util.List.of(
                        lang.getMessage(
                            "gui.container.manage_report.back_button.lore",
                            Map.of(
                                "status",
                                lang.getLocalizedStatus(report.getStatus())
                            )
    ...
    private String parsePlaceholders(Player player, String message) {
        return MessageUtils.parsePlaceholders(player, message);
    }

    private java.util.List<String> parsePlaceholders(
        Player player,
        java.util.List<String> messages
    ) {
        return MessageUtils.parsePlaceholders(player, messages);
    }
    ...
    private void openBookGUI(Player player, Report report) {
        ...
        class BookPaginator {
            ...
            java.util.List<
                java.util.List<net.md_5.bungee.api.chat.BaseComponent>
            > pages = new java.util.ArrayList<>();
            java.util.List<net.md_5.bungee.api.chat.BaseComponent> currentPage =
                ...
                if (rawText.equals("\n")) {
                    ...
                    for (String line : stripped.split("\n", -1)) {
                        ...
                        estimatedLines += Math.max(
                            1,
                            (int) Math.ceil(line.length() / 23.0)
                        );
                    }
                }
            ...
            net.md_5.bungee.api.chat.BaseComponent[][] getPagesArray() {
                ...
                net.md_5.bungee.api.chat.BaseComponent[][] array =
                    new net.md_5.bungee.api.chat.BaseComponent[pages.size()][];
                for (int i = 0; i < pages.size(); i++) {
                    array[i] = pages
                        .get(i)
                        .toArray(new net.md_5.bungee.api.chat.BaseComponent[0]);
                }
                return array;
            }
        }
        ...
        paginator.add(createLegacy(player, titleText), titleText);

        String reporterText = lang.getMessage(
            "gui.book.page.reporter",
            Map.of("reporter", reporterName != null ? reporterName : "Unknown")
        );
        paginator.add(createLegacy(player, reporterText), reporterText);

        String reportedText = lang.getMessage(
            "gui.book.page.reported",
            Map.of("reported", reportedName != null ? reportedName : "Unknown")
        );
        paginator.add(createLegacy(player, reportedText), reportedText);

        String reasonPrefixText = lang.getMessage("gui.book.page.reason");
        paginator.add(createLegacy(player, reasonPrefixText), reasonPrefixText);

        String reasonHoverText = lang.getMessage("gui.book.page.reason_hover");
        paginator.add(
            createInteractiveLegacy(
                player,
                reasonHoverText,
                null,
                lang.getMessage(
                    "gui.book.page.reason_hover_content",
                    Map.of(
                        "reason",
                        lang.getLocalizedReason(report.getReason())
                    )
        ...
        String statusText = lang.getMessage(
            "gui.book.page.status",
            Map.of("color", statusColor, "status", statusName)
        );
        paginator.add(createLegacy(player, statusText), statusText);

        String idText = lang.getMessage(
            "gui.book.page.id",
            Map.of("id", String.valueOf(report.getId()))
        );
        paginator.add(createLegacy(player, idText), idText);

        if (plugin.getDatabaseManager().hasMultipleServers()) {
            String serverName = report.getServerName();
            if (serverName == null || serverName.isEmpty()) serverName =
                lang.getMessage("common.unknown");
            String serverText = lang.getMessage(
                "gui.book.page.server",
                Map.of("server", serverName)
            );
        ...
        if (player.hasPermission("aevorinreports.manage")) {
            String clickChangeText = lang.getMessage(
                "gui.book.page.click_to_change"
            );
            paginator.add(
                createLegacy(player, clickChangeText),
                clickChangeText
            );

            if (report.getStatus() != Report.ReportStatus.PENDING) {
                String pendingText = lang.getMessage(
                    "gui.book.status_options.pending"
                );
                paginator.add(
                    createInteractiveLegacy(
                        player,
            ...
            if (report.getStatus() != Report.ReportStatus.RESOLVED) {
                String resolvedText = lang.getMessage(
                    "gui.book.status_options.resolved"
                );
                paginator.add(
                    createInteractiveLegacy(
                        player,
            ...
            if (report.getStatus() != Report.ReportStatus.REJECTED) {
                String rejectedText = lang.getMessage(
                    "gui.book.status_options.rejected"
                );
                paginator.add(
                    createInteractiveLegacy(
                        player,
    ...
    private net.md_5.bungee.api.chat.TextComponent createLegacy(
        Player player,
        String text
    ) {
        text = MessageUtils.parsePlaceholders(player, text);
        net.md_5.bungee.api.chat.TextComponent parent =
            new net.md_5.bungee.api.chat.TextComponent("");
        for (net.md_5.bungee.api.chat.BaseComponent c : net.md_5.bungee.api.chat.TextComponent.fromLegacyText(
    ...
    private net.md_5.bungee.api.chat.TextComponent createInteractiveLegacy(
        Player player,
        String text,
        String command,
        String hoverText
    ) {
        net.md_5.bungee.api.chat.TextComponent parent = createLegacy(
            player,
            text
        );
        if (command != null) {
            parent.setClickEvent(
        ...
        if (hoverText != null) {
            hoverText = MessageUtils.parsePlaceholders(player, hoverText);
            parent.setHoverEvent(
